...
4. **Grill (if needed).** If the request needs fleshing out, read [grilling](../../productivity/grilling/SKILL.md) and apply relevant interview rounds until the ticket's decisions are sufficient. Consult the existing glossary for terminology; call "domain-modeling" to update `CONTEXT.md`/ADRs only if document maintenance is authorized.
