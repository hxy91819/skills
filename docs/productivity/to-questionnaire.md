...
No, and the distinction is worth holding. `grill-me` already asks in **rounds**: a relevant, manageable part of the frontier, then recomputed from your answers. It groups questions without promising to ask every available question at once. `to-questionnaire` is about a different axis: not how the questions are delivered, but whose head the answers are in. Answering them yourself faster is `grill-me`; getting them out of someone else is this.
