...
| --- | --- | --- | --- |
| `grilling` | HITL | The default. The question can be settled by talking it through. | The [grilling](https://aihero.dev/skills-grilling) reference; [domain-modeling](https://aihero.dev/skills-domain-modeling) only within authorized document maintenance |
| `prototype` | HITL | "How should this look" or "how should this behave": a question talking cannot settle. | [prototype](https://aihero.dev/skills-prototype) when an interactive artifact is explicitly requested; link an authorized asset and record the human's verdict |
| `research` | AFK | A fact outside the working directory is blocking a decision. | Direct primary-source investigation with cited findings on the ticket; bounded parallel reading is optional when permitted |
...
Underneath, it reads [grilling](https://aihero.dev/skills-grilling) for interviews and uses [domain-modeling](https://aihero.dev/skills-domain-modeling) only when document maintenance is authorized. Explicitly requested [prototype](https://aihero.dev/skills-prototype) tasks produce local artifacts; research tickets use direct primary-source investigation, with optional bounded delegation. Neither requires a published branch. [handoff](https://aihero.dev/skills-handoff) bridges sessions; [ask-matt](https://aihero.dev/skills-ask-matt) maps the whole set.