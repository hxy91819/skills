...
When a report needs an interview, the flow reads the [grilling](https://aihero.dev/skills-grilling) reference until the ticket has enough decisions. Updating domain documents requires that maintenance to be in the authorized task; explaining terms alone does not write files.

## Where it fits

`triage` is an **on-ramp**, not a step in the main chain. The main flow runs from an idea you had (grill, spec, tickets, implement, review), and `triage` is the parallel lane for work that arrived instead. It merges at the same place: an issue labelled `ready-for-agent` with a brief on it, which [implement](https://aihero.dev/skills-implement) picks up exactly as it would a ticket from [to-tickets](https://aihero.dev/skills-to-tickets). When a request needs sharpening before it can be briefed, `triage` reads [grilling](https://aihero.dev/skills-grilling) for the interview and uses [domain-modeling](https://aihero.dev/skills-domain-modeling) only when maintaining domain documents is authorized. When you're not sure which lane you are in, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
